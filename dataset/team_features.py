...
class TeamFeatures:
    ...
    def _normalize_features(self, feats):
        ...
        norm_features = feats.groupby('matchday', group_keys=False).apply(self._scale_group)
        # ONly for testing! REMOVE feats[cols_to_drop]
        return pd.concat([config, feats[cols_to_drop], norm_features[self.met_col_list]], axis=1)
